...
- **🔴🔴🔴 手动创建 UID 文件（最严重错误！）**：
  - **症状**：场景无法加载，`Condition "!int_resources.has(id)" is true`
  - **原因**：手动创建 `.uid` 文件（54个文件）和场景 UID
  - **影响**：场景完全无法加载，Godot 无法解析资源
  - ✅ **预防**：永远不要手动创建 `.uid` 文件，只让 Godot 自动生成
  - ✅ **修复**：`find . -name "*.uid" -type f -delete`，删除所有手动 UID
  - ✅ **理解**：
    - `.uid` 文件只为脚本自动生成（`.gd.uid`）
    - 场景文件不需要 `.uid` 文件
    - 场景 UID 在 `.tscn` 文件内部，由编辑器自动管理
  - 📄 **详细文档**：[UID手动创建错误复盘.md](./UID手动创建错误复盘.md)
...
- **场景文件使用 GDScript 方法**：`.tscn` 文件中不能使用方法调用
  - **症状**：`Parse Error: Invalid parameter`，场景无法加载
  - **原因**：在场景文件中写了 `set_border_width_all(2)` 等方法调用
  - ✅ **预防**：场景文件只使用 `property = value` 格式
  - ✅ **修复**：将方法调用替换为独立属性赋值
...
2. **[UID不匹配问题修复指南.md](./UID不匹配问题修复指南.md)** - UID 不匹配（文件移动后）问题详解
3. **[UID手动创建错误复盘.md](./UID手动创建错误复盘.md)** - 🔴 手动创建 UID 文件的严重错误复盘
4. **[场景文件移动后的缓存清理问题.md](./场景文件移动后的缓存清理问题.md)** - 缓存问题解决方案
5. **[项目文件组织规范.md](./项目文件组织规范.md)** - 文件组织标准
6. **[UI开发问题汇总.md](./UI开发问题汇总.md)** - UI 开发中的所有问题汇总
