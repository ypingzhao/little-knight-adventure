# 项目文件组织规范

## 📋 概述
本文档定义了 Little Knight Adventure 项目的文件组织规范，确保项目结构清晰、易于维护和扩展。

## ⚠️ 重要提示：Windows 文件系统不区分大小写

**问题**：在 Windows 系统中，`Scripts/` 和 `scripts/` 被视为同一个文件夹！

**影响**：
- 删除 `Scripts/` 会同时删除 `scripts/`
- 重命名操作可能会影响大小写不同的同名文件夹

**解决方案**：
1. 使用不同的文件夹名（推荐）
2. 或在删除前确认文件夹实际名称
3. 使用 Git 操作避免误删

**本项目约定**：
- ✅ 使用小写 `scripts/` 作为脚本文件夹
- ❌ 不使用 `Scripts/` 等大写开头名称

---

## 📁 推荐的项目结构

```
little_knight_adventure_v0.5/
├── assets/                    # 游戏资源文件（不包含代码）
│   ├── audio/                 # 音效和音乐
│   ├── fonts/                 # 字体文件
│   ├── images/                # 图片资源
│   ├── shaders/               # 着色器
│   └── textures/              # 纹理图集
│
├── scripts/                   # 所有脚本文件（统一小写）
│   ├── autoload/              # 自动加载的单例脚本
│   │   ├── global_data.gd
│   │   ├── save_system.gd
│   │   └── trade_item_list.gd
│   │
│   ├── actors/                # 角色/实体脚本
│   │   ├── player/            # 玩家相关
│   │   │   ├── player.gd
│   │   │   ├── player_health.gd
│   │   │   └── states/        # 玩家状态机
│   │   │       ├── player_state.gd
│   │   │       ├── idle.gd
│   │   │       ├── run.gd
│   │   │       └── jump.gd
│   │   │
│   │   ├── enemies/           # 敌人脚本
│   │   │   ├── bat.gd
│   │   │   ├── slime.gd
│   │   │   ├── slime_boss.gd
│   │   │   └── bat_spawner.gd
│   │   │
│   │   └── npc/               # NPC脚本
│   │       ├── shop_keeper.gd
│   │       ├── npc.gd
│   │       └── knight.gd
│   │
│   ├── components/            # 可复用组件
│   │   ├── health_component.gd
│   │   ├── damage_area.gd
│   │   └── interaction_area.gd
│   │
│   ├── items/                 # 道具/物品脚本
│   │   ├── coin.gd
│   │   ├── health_potion.gd
│   │   ├── door.gd
│   │   ├── sword.gd
│   │   └── treasure_box.gd
│   │
│   ├── managers/              # 管理器脚本
│   │   ├── level_manager.gd
│   │   ├── game_manager.gd
│   │   └── audio_manager.gd
│   │
│   ├── traps/                 # 陷阱脚本
│   │   ├── spike.gd
│   │   ├── waterfall.gd
│   │   ├── platform.gd
│   │   └── falling_block.gd
│   │
│   ├── ui/                    # UI相关脚本
│   │   ├── hud.gd
│   │   ├── shop_ui.gd
│   │   ├── chat_bubble.gd
│   │   └── menu_ui.gd
│   │
│   ├── utils/                 # 工具脚本
│   │   └── create_animation.gd
│   │
│   └── levels/                # 关卡脚本
│       ├── shop.gd
│       ├── dungeon.gd
│       └── boss_arena.gd
│
├── scenes/                    # 场景文件（.tscn）
│   ├── actors/                # 角色场景
│   │   ├── player/
│   │   ├── enemies/
│   │   └── npc/
│   │
│   ├── items/                 # 道具场景
│   ├── levels/                # 关卡场景
│   ├── traps/                 # 陷阱场景
│   ├── ui/                    # UI场景
│   └── components/            # 组件场景
│
├── tilesets/                  # 瓦片集资源
│   ├── dungeon.tres
│   ├── forest.tres
│   └── castle.tres
│
├── resources/                 # 其他资源文件
│   ├── themes/                # UI主题
│   └── presets/               # 预设配置
│
├── references/                # 开发文档
│   ├── 项目文件组织规范.md
│   ├── 商店系统实现文档.md
│   └── 2D动画生成注意事项.md
│
├── .gitignore                 # Git忽略配置
├── export_presets.cfg         # 导出预设
├── project.godot              # Godot项目配置
└── README.md                  # 项目说明
```

---

## 📝 命名规范

### 文件夹命名
- ✅ 使用**小写字母和下划线**
- ✅ 使用复数形式（`actors/` 而非 `actor/`）
- ✅ 描述性名称（`enemies/` 而非 `data/`）

**示例**:
```
✅ scripts/actors/player/
✅ scripts/managers/
✅ scripts/ui/
❌ Scripts/Actors/Player/
❌ scripts/Manager/
❌ scripts/data/
```

### 脚本文件命名
- ✅ 使用**小写字母和下划线**
- ✅ 类名使用 PascalCase（文件名和类名对应）
- ✅ 描述性名称

**示例**:
```gdscript
# 文件名: player_health.gd
class_name PlayerHealth extends Node

# 文件名: slime_boss.gd
class_name SlimeBoss extends CharacterBody2D
```

### 场景文件命名
- ✅ 与脚本同名（如果有）
- ✅ 描述性名称
- ✅ 使用下划线分隔

**示例**:
```
player.tscn          # 对应 player.gd
slime_boss.tscn      # 对应 slime_boss.gd
shop_ui.tscn         # 对应 shop_ui.gd
```

---

## 🎯 文件分类规则

### 1. 脚本分类 (`scripts/`)

#### actors/ - 角色/实体脚本
- **用途**: 游戏中的主要角色（玩家、敌人、NPC）
- **包含**: 继承自 CharacterBody2D、Area2D 的角色
- **示例**: `player.gd`, `bat.gd`, `shop_keeper.gd`

#### components/ - 可复用组件
- **用途**: 可被多个对象复用的功能模块
- **包含**: 生命值组件、交互区域、伤害区域
- **示例**: `health_component.gd`, `interaction_area.gd`

#### items/ - 道具/物品
- **用途**: 可拾取、可使用的物品
- **包含**: 金币、药水、武器、门
- **示例**: `coin.gd`, `health_potion.gd`, `sword.gd`

#### managers/ - 管理器
- **用途**: 管理游戏系统
- **包含**: 关卡管理、音频管理、保存系统
- **示例**: `level_manager.gd`, `audio_manager.gd`

#### ui/ - 用户界面
- **用途**: 所有UI相关脚本
- **包含**: HUD、菜单、对话框
- **示例**: `hud.gd`, `shop_ui.gd`, `chat_bubble.gd`

#### traps/ - 陷阱
- **用途**: 关卡中的危险物体
- **包含**: 刺、落石、移动平台
- **示例**: `spike.gd`, `falling_block.gd`, `moving_platform.gd`

#### levels/ - 关卡脚本
- **用途**: 关卡逻辑和控制
- **包含**: 关卡初始化、事件触发
- **示例**: `shop.gd`, `dungeon.gd`, `boss_arena.gd`

### 2. 场景分类 (`scenes/`)

场景文件的分类应该与脚本分类保持一致：

```
scenes/
├── actors/        # player/, enemies/, npc/
├── items/         # coins/, potions/, weapons/
├── levels/        # 关卡场景
├── traps/         # spike/, platforms/
└── ui/            # hud/, menus/
```

### 3. 资源分类 (`assets/`)

```
assets/
├── audio/         # sound_effects/, music/
├── textures/      # characters/, items/, tilesets/
├── fonts/         # 字体文件
└── shaders/       # 着色器代码
```

---

## 🔄 迁移规则

### 当前问题
- 根目录有大量脚本文件
- Scripts/ 和 scenes/ 下脚本重复
- 文件分散，难以维护

### 迁移步骤
1. **创建新的文件夹结构**
2. **移动脚本文件**（保持场景文件不变）
3. **更新场景文件中的脚本引用**
4. **更新 autoload 配置**
5. **测试所有场景**

### 重要注意事项

#### ⚠️ 不要移动场景文件
场景文件（`.tscn`）保持原位置，只移动脚本文件。

#### ⚠️ 更新脚本路径
移动脚本后，必须更新场景文件中的 `ext_resource` 引用：

```tscn
# 之前
[ext_resource type="Script" path="res://shop_keeper.gd" id="shop_keeper_script"]

# 之后
[ext_resource type="Script" path="res://scripts/actors/npc/shop_keeper.gd" id="shop_keeper_script"]
```

#### ⚠️ 更新 autoload 配置
如果移动了 autoload 脚本，需要更新 `project.godot`：

```ini
[autoload]
GlobalData="*res://scripts/autoload/global_data.gd"
SaveLoad="*res://scripts/autoload/save_system.gd"
```

---

## 📋 具体迁移计划

### 根目录脚本 → `scripts/`

| 当前位置 | 新位置 | 说明 |
|---------|--------|------|
| `Global.gd` | `scripts/autoload/global_data.gd` | 自动加载 |
| `area_2d.gd` | `scripts/components/interaction_area.gd` | 交互区域 |
| `box.gd` | `scripts/items/box.gd` | 宝箱 |
| `coco_adventure.gd` | `scripts/levels/coco_adventure.gd` | 关卡脚本 |
| `door.gd` | `scripts/items/door.gd` | 门 |
| `dungeon.gd` | `scripts/levels/dungeon.gd` | 关卡脚本 |
| `fall_box.gd` | `scripts/traps/falling_box.gd` | 陷阱 |
| `fall_stone.gd` | `scripts/traps/falling_stone.gd` | 陷阱 |
| `flying_pig.gd` | `scripts/actors/npc/flying_pig.gd` | NPC |
| `kill_zone.gd` | `scripts/traps/kill_zone.gd` | 陷阱 |
| `slime_green.gd` | `scripts/actors/enemies/slime_green.gd` | 敌人 |
| `treasure_box.gd` | `scripts/items/treasure_box.gd` | 道具 |

### Scripts/ 子目录重组

| 当前位置 | 新位置 |
|---------|--------|
| `Scripts/HUD.gd` | `scripts/ui/hud.gd` |
| `Scripts/coin.gd` | `scripts/items/coin.gd` |
| `Scripts/chat_bubble.gd` | `scripts/ui/chat_bubble.gd` |
| `Scripts/shop_ui.gd` | `scripts/ui/shop_ui.gd` |
| `Scripts/shop_keeper.gd` | `scripts/actors/npc/shop_keeper.gd` |
| `Scripts/shop.gd` | `scripts/levels/shop.gd` |
| `Scripts/knight.gd` | `scripts/actors/player/knight.gd` |
| `Scripts/start_game.gd` | `scripts/ui/start_menu.gd` |
| `Scripts/health_pot.gd` | `scripts/items/health_potion.gd` |
| `Scripts/sword.gd` | `scripts/items/sword.gd` |
| `Scripts/bat.gd` | `scripts/actors/enemies/bat.gd` |
| `Scripts/bat_spawner.gd` | `scripts/actors/enemies/bat_spawner.gd` |
| `Scripts/slime_purple_boss.gd` | `scripts/actors/enemies/slime_boss.gd` |
| `Scripts/platform_a_active.gd` | `scripts/traps/active_platform.gd` |
| `Scripts/oscar_adventure.gd` | `scripts/levels/oscar_adventure.gd` |
| `Scripts/tools/create_king_animation.gd` | `scripts/utils/create_king_animation.gd` |

### player/ 文件夹重组

| 当前位置 | 新位置 |
|---------|--------|
| `player/scripts/player.gd` | `scripts/actors/player/player.gd` |
| `player/scripts/player_health.gd` | `scripts/actors/player/player_health.gd` |
| `player/states/*.gd` | `scripts/actors/player/states/*.gd` |

### scenes/ 下的脚本

| 当前位置 | 新位置 |
|---------|--------|
| `scenes/NPC/npc.gd` | `scripts/actors/npc/npc.gd` |
| `scenes/traps/spike.gd` | `scripts/traps/spike.gd` |
| `scenes/traps/waterfall.gd` | `scripts/traps/waterfall.gd` |
| `scenes/boss_01_scene.gd` | `scripts/levels/boss_arena.gd` |

---

## ✅ 验证清单

完成迁移后，检查以下内容：

- [ ] 所有场景文件正常打开
- [ ] 所有脚本引用正确更新
- [ ] autoload 配置正确
- [ ] 游戏可以正常运行
- [ ] 所有关卡正常加载
- [ ] NPC和敌人正常工作
- [ ] UI功能正常

---

## 🎯 维护原则

### 1. 一致性原则
- 同类文件放在同一目录
- 命名风格统一
- 脚本和场景对应

### 2. 可查找性
- 按功能分类，不是按实现方式
- 使用描述性名称
- 避免深层嵌套（最多3-4层）

### 3. 可扩展性
- 为未来内容预留空间
- 添加新类型时明确分类
- 定期review和重构

---

**文档版本**: 1.0
**最后更新**: 2025-02-11
**适用项目**: Little Knight Adventure v0.5
